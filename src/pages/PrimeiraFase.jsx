...
export default function PrimeiraFase() {
  ...
  function MatchRow({ match }) {
    ...
    const inputCls = (locked) =>
      `w-8 h-7 md:w-9 md:h-8 text-center text-sm font-semibold border rounded-lg transition ${
        locked
    ...
    if (noPrediction) {
      return (
        <div className={`transition ${result ? ROW_STYLE[result] : ''} opacity-60`}>
          <div className="flex items-center gap-2 md:gap-4 px-3 md:px-5 pt-2.5 pb-1 md:py-2.5">
            ...
            <div className="shrink-0 flex items-center gap-1 text-sm font-semibold text-gray-500 w-16 md:w-20 justify-center">
    ...
    return (
      <div className={`transition ${result ? ROW_STYLE[result] : 'hover:bg-gray-50'}`}>
        ...
        {/* Data/grupo — só mobile, centralizado abaixo */}
        <div className="md:hidden text-center pb-2">
          <span className="text-xs text-gray-400 capitalize">
            {formatDate(match.match_date)} · Grupo {match.group_name}
          </span>
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-gray-50 py-8">
      <div className="max-w-5xl mx-auto px-4 md:px-8">
        ...
        <div className="flex items-start justify-between mb-6 gap-3">
          ...
          <div className="bg-white border border-gray-200 rounded-xl px-3 py-2 md:px-6 md:py-4 text-right shrink-0">
            <p className="text-xs text-gray-400 mb-0.5">Minha pontuação total</p>
            <p className="text-lg md:text-3xl font-bold text-gray-900 leading-none">
              {totalPontos} <span className="text-xs md:text-sm font-normal text-gray-400">pts</span>
            </p>
          </div>
        </div>
        ...
        {/* Resumo — oculto no mobile */}
        <div className="hidden md:block bg-white border border-gray-200 rounded-2xl px-5 py-4 mb-4">
